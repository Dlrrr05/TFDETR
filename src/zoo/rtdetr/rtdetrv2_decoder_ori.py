...
class RTDETRTransformerv2(nn.Module):
    ...
    def _get_decoder_input(self,
        ...
        if self.training or self.eval_spatial_size is None:
            ...
            # IMPORTANT: if cached anchors/mask length mismatches current memory, rebuild from current spatial_shapes
            if anchors.shape[1] != memory.shape[1]:
                anchors, valid_mask = self._generate_anchors(spatial_shapes, device=memory.device, dtype=memory.dtype)

        # align batch for mask (anchors stays [1, L, 4] and will broadcast over batch)
